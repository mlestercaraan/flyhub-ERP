        ...
        <div class="card shadow">
            ...
            <div class="d-flex align-items-center">
              <form class="search-bar me-3" method="get" action="">
                <div class="input-group input-group-sm">
                  <input type="text" name="search" value="<?php echo htmlspecialchars($searchTerm); ?>" class="form-control" placeholder="Search contacts...">
          ...
          <div class="card-body p-0">
            <?php
              ...
              $allowed_cols = ['first_name', 'last_name', 'email', 'phone'];
              if (!in_array($order_by, $allowed_cols)) $order_by = 'first_name';
              $where = '';
              if ($searchTerm !== '') {
                $s = $conn->real_escape_string($searchTerm);
                $where = "WHERE first_name LIKE '%$s%' OR last_name LIKE '%$s%' OR email LIKE '%$s%' OR phone LIKE '%$s%'";
              }
              $sql = "SELECT * FROM contacts $where ORDER BY $order_by $order_dir";
                  ...
                  <!-- Table Head -->
                    <thead>
                      <tr>
                        ...
                        <th>
                          <?php
                            // Sort link for Name (use first_name for sorting, you can expand this later)
                            echo sort_link('first_name','Name',$order_by,$order_dir,$searchTerm);
                          ?>
                        </th>
                        <th>
                          <?php echo sort_link('email','Email',$order_by,$order_dir,$searchTerm); ?>
                        </th>
                        <th><?php echo sort_link('phone','Phone',$order_by,$order_dir,$searchTerm); ?></th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                    <?php
                      if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                          ...
                          echo "<tr data-id='{$contact_id}'>
                            <td><input type='checkbox' name='selected_ids[]' value='{$contact_id}' class='selectbox'></td>
                            <td>
                              <a href='profile.php?id={$contact_id}' class='fw-bold text-decoration-none text-primary'>{$full_name}</a>
                            </td>
                            <td>
                              <a href='profile.php?id={$contact_id}' class='text-decoration-none text-primary'>{$email}</a>
                            </td>
                            <td contenteditable='true' class='editable' data-field='phone'>{$phone}</td>
                            <td>
                              <a href='edit.php?id={$contact_id}' class='btn btn-sm btn-warning me-1'>Edit</a>
                              <a href='delete.php?id={$contact_id}' class='btn btn-sm btn-danger'
